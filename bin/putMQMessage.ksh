...
{
   if [ -z "$PROFILE" ]; then
      echo "Usage: putMQMessage <profileName> <Queue Name> <Message File>"
      exit
   fi
}
#-----------------------------------#
...
#-----------------------------------#
{
   ...
   ${JAVA_HOME}/bin/java MQWrite -h ${QMGR_HOSTNAME} -p ${QMGR_PORT} -c ${QMGR_CHANNEL} -m ${QMGR_NAME} -q ${QUEUE} -f ${MESSAGEFILE}
   return $?
   echo ""
}
#-----------------------------------#
